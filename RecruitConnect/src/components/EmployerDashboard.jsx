...
const EmployerDashboard = () => {
    ...
    const fetchEmployerData = async () => {
      try {
        ...
        const response = await axios.get(
          `http://127.0.0.1:5000//jobs/<int:employer_id>/${employer_id}`,
          {
            headers: {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case "Job Listings":
        return <JobPostingForm />; // Added a case to render JobPostingForm for Job Listings tab
      // Add other cases as needed for the other tabs
      default:
        return <Overview stats={employerData?.stats} />;
    }
  };
